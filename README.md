...
2. Repository name: `nrt-forest-monitor` (or similar)
3. Description: `Near-real-time forest change detection using Sentinel-2 and CCDC (Google Earth Engine)`
4. Set to **Public**
5. Check **"Add a README file"** → we will replace it
...
|---|---|
| `README.md` | This file |
| `nrt-monitoring-demo.js` | Main script |
| `ccdc-utils.js` | CCDC helper module |
| `reference_polygons.geojson` | Validation polygons |
| `s2_animation.gif` | S2 time series animation |
| `dnbr_animation.gif` | NRT output animation |
| `Illustration 1.png` | Detection overview |
| `Illustration 2.png` | RdNBR close-up |
| `Illustration 3.png` | CCDC synthetic reference |

**Files NOT to include** (keep private):
- `temporalSegmentation.js` — original library, stays with daniel.wiell's repo
- `Manuscript_revised_track_changes.docx`
- `s2_images_gif/` and `dnbr_images_gif/` source frame folders *(optional)*

**Upload via browser:**
```
On your new repo page → "Add file" → "Upload files" → drag and drop all files above
```

**Or via git:**
```bash
git clone https://github.com/<your-username>/nrt-forest-monitor.git
cd nrt-forest-monitor
# copy files here, then:
git add .
git commit -m "Initial release: NRT forest change detection demo"
git push origin main
```

### Step 3 — Add a GEE script link

1. In GEE Code Editor, save your script
2. Click **"Get Link"** (top right) → copy the URL
3. Paste it into the README under "How to run → Option A"

### Step 4 — Add repository topics (makes it discoverable)

On the GitHub repo page → click the gear ⚙ icon next to "About":
```
Topics: google-earth-engine, sentinel-2, forest-monitoring, ccdc, near-real-time, remote-sensing, ukraine
```

### Step 5 — Link from the paper

Once the repo is public, add the GitHub URL to your paper's data availability statement:
```
Code available at: https://github.com/<your-username>/nrt-forest-monitor
```
